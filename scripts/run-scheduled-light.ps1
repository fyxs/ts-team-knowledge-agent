# Light lane: copy md / transcode txt / split xlsx without waiting behind MinerU heavy jobs.
# Runs with its own light.lock (independent from the heavy run.lock), so a long PDF no longer
# blocks newly added markdown from being indexed.
# Called by scheduled task TSKnowledgeAgentSchedulerLight (offset 2 minutes from the main task).
...
$errorLog = Join-Path $root 'logs\runner-errors-light.log'
...
Set-Location $root
$log = Join-Path $logDir 'scheduled-run-light.log'
& cmd.exe /c ('"' + $cli + '" run-once --lane light --sync --if-due --batch-size 25 >> "' + $log + '" 2>&1')
exit $LASTEXITCODE
