...
**监听、看门狗、自愈、台账、审计、闸口这类脚本属于我们自己的运维设施，只允许存在于
mis/mos/mes 的 Hermes 环境里；不得进入发布件，也不得进入用户/成员主机。**

理由：用户主机上只应存在"应用本身"。一旦运维脚本随包分发，用户主机会变成被监控对象，
责任边界与信任关系都会被破坏。

落实：

- 发布前把"包内容核对"当作发布检查项：发布件里不得出现 `.py` 脚本，也不得出现
  watch / monitor / cron / task / attempt / ledger / hook 等运维类文件
  （第三方依赖自带的同名库不算，需逐一确认来源）；
- 发布件只允许有：应用可执行文件、必要的助手二进制、使用说明与依赖清单；
- 应用自身的埋点（usage）属于产品设计、经用户批准，写本地并经共享仓汇总，
  与"运维脚本"是两回事，不要混为一谈；
- 新增运维脚本时，落点固定为各机 Hermes 环境（mis/mos/mes），并在文档里写明它跑在哪台机器。
