...
落点       D:\2Work\Knowledge\TS-Share\飞书导出\<一级分类>\<文档标题>\<文档标题>.md
           图片/附件：同目录下 <文档标题>.assets\asset-NN.<ext>
           —— **每篇文档一个自己的目录**，md 与其 assets 在一起，人工一眼可对应
布局       固定三级（一级分类 / 文档标题 / 正文）。实测结论：
           · 变量深度 + 深层嵌套 → 递归枚举与逐路径可见性不一致（六种枚举只看到 28/54）
           · 两级（分类/标题.md 与 标题.assets 并列）→ 枚举一致，但多篇同分类时
             md 与 assets 分不清归属（用户反馈）
           · 三级（每篇自带目录）→ 枚举 54/54 一致 ✓ 且归属清晰 ✓（当前方案）
